$(function () {
    ...
    $(document).on('change', '#add_type', function(){
        ...
        if (type_id == 6) {
            ...
            $('#value_input').html('<div class="row">'+
                        '<div class="col-md-6">'+
                            '<button type="button" class="btn btn-info app-html-preview form-control" data-html="add_value">Preview</button>'+
                            '<input type="hidden" name="add_value" id="add_value" value="">'+
                        '</div>'+
                        '<div class="col-md-6">'+
                            '<button type="button" class="btn btn-warning app-html-edit form-control" data-html="add_value">Edit HTML</button>'+
                        '</div>'+
                    '</div>')
        }else{
            $('#value_input').html('<input type="text" name="add_value" id="add_value" class="form-control" value="" placeholder="Value">');
    ...
    $(document).on('click', '.app-html-preview', function(){
    ...
    $(document).on('click', '.app-save-html', function(){
        var field = $(this).data('html');
    ...
    $(document).on('click', '.app-html-edit', function(){
        var field = $(this).data('html');

        var html = $("#"+field).val();

        $(".html-modal-title").html(field);
        $(".app-save-html").data("html", field);
        $(".app-save-html").show();
        $(".tinymce").html(html);
        ...
        tinymce.init({
            ...
            setup: function (editor) {
                editor.addButton('media_local', {
                    ...
                    icon: false,
                    onclick: function () {
                        AppMedia.loadMedia('blog', 'post_content');
                    }
                });
                if(hasCloudinary){
                    editor.addButton('cloudinary', {
                        ...
                        icon: false,
                        onclick: function () {
                            AppCloudinary.loadCloudinary('blog', 'post_content');
                        }
                    });
                }
