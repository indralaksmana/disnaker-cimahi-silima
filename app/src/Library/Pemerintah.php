...
class Pemerintah extends Library
{
    ...
    public function addPost()
    {
        ...
        if ($this->validator->isValid()) {
            ...
            if ( $user ) {
                $post = new PemerintahModel;
                $post->user_id = $user->id;
                $post->id = $user->id;
                $post->nama_instansi = $r['nama_instansi'];
                $post->jenis_instansi_id = $r['jenis_instansi'];
                $post->nama_lengkap_admin = $r['fullname'];
                $post->save();
    ...
    public function updatePost($postId)
    {
        ...
        if ($this->validator->isValid()) {
            $user = $this->update_user( $r['user_id'] );
            if ( $user ) {
                $post = new PemerintahModel;
    ...
    {
        $post = U::find($this->container->request->getParam('post_id'));
    ...
    {
        $post = LpkModel::find($this->container->request->getParam('post_id'));
    ...
    {
        $slug = Utils::slugify($this->container->request->getParam('nama_lpk'));
        $checkSlug = LpkModel::where('slug', $slug);
        if ($postId) {
            $checkSlug = $checkSlug->where('id', '!=', $postId);
        }
        if ($checkSlug->first()) {
            $this->validator->addError('title', 'Nama LPK Sudah Di Pakai');
            return false;
        }
        $this->slug = $slug;
        return true;
    ...
    public function update_user( $user_id = null )
    {
        ...
        if ( $user_id ) {
            ...
            $role = $this->container->auth->findRoleByName('Pemerintah');
