...
class Pendidikan extends Library
{
    ...
    public function addPost()
    {
        ...
        if ($this->validator->isValid()) {
            $newPost = new PendidikanPosts;
            $newPost->level = $requestParams['level'];
            $newPost->schoolname = $requestParams['schoolname'];
            $newPost->schoolmajors = $requestParams['schoolmajors'];
            $newPost->graduationyear = $requestParams['graduationyear'];
            $newPost->schoolgrades = $requestParams['schoolgrades'];
            $newPost->numberofdiplomas = $requestParams['numberofdiplomas'];
    ...
    public function updatePost($postId)
    {
        ...
        //Check Post
        $post = PendidikanPosts::find($postId);
        ...
        if ($this->validator->isValid()) {
            ...
            $post->schoolname = $requestParams['schoolname'];
            $post->schoolmajors = $requestParams['schoolmajors'];
            $post->graduationyear = $requestParams['graduationyear'];
            $post->schoolgrades = $requestParams['schoolgrades'];
            $post->numberofdiplomas = $requestParams['numberofdiplomas'];
    ...
    {
        $post = PendidikanPosts::find($this->container->request->getParam('post_id'));
    ...
    private function validateTitleDesc($postId = null)
    {
        ...
        // $checkTitle = PendidikanPosts::where('title', $this->container->request->getParam('title'));
        // if ($postId) {
