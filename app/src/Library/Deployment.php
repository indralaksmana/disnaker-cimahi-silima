...
/**
 * This is the Git Auto-Deployment class for the App Framework.  This class
 * provides usage for checking for all necessary dependencies and automatically
 * installing them.  This is meant to deploy a App framework project, but I
...
class Deployment
{
    ...
    private function validateDocumentRoot()
    {
        ...
        if (!is_writable(dirname($this->document_root))) {
            die($this->logEntry("Web server user does not have access to the DOCUMENT_ROOT's parent directory. ".
                "This is required in order for App to function properly."));
        }
    }
    ...
    private function checkSettings()
    {
        ...
        if (!is_file(dirname($this->document_root) . '/settings.json')) {
            $this->logEntry("App settings.json not found.  Creating now...");
            ...
            $settings_file = file_get_contents(
                "https://raw.githubusercontent.com/App/framework/master/settings.dist.json"
            );
