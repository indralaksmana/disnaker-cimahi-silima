...
class PerguruanTinggi extends Library
{
    ...
    {
        $post = PerguruanTinggiModel::find($this->container->request->getParam('post_id'));
    ...
    {
        $slug = Utils::slugify($this->container->request->getParam('nama_perguruan_tinggi'));
        $checkSlug = PerguruanTinggiModel::where('slug', $slug);
        if ($postId) {
            $checkSlug = $checkSlug->where('id', '!=', $postId);
        }
        if ($checkSlug->first()) {
            $this->validator->addError('title', 'Nama Perguruan Tinggi Sudah Di Pakai');
            return false;
        }
        $this->slug = $slug;
        return true;
    ...
    public function update_user( $user_id = null )
    {
        ...
        if ( $user_id ) {
            ...
            $user->perguruan_tinggi_id = $this->container->request->getParam('perguruan_tinggi_id');
            $user->save();
            ...
            $userDetails = [
                ...
                'perguruan_tinggi_id' => $this->container->request->getParam('perguruan_tinggi_id'),
                'permissions' => [
                    'user.delete' => 0
                ]
            ];

            $role = $this->container->auth->findRoleByName('Perguruan Tinggi');
