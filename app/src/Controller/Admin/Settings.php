...
use App\Library\Settings as S;
use App\Model\Config;
use App\Model\ConfigGroups;
use App\Model\ConfigTypes;
...
class Settings extends Controller
{
    ...
    public function settingsGlobal(Request $request, Response $response)
    {
        ...
        return $this->view->render(
            $response,
            'dashboard/admin/settings/settings-global.twig',
            array(
    ...
    public function save(Request $request, Response $response)
    {
        ...
        foreach ($request->getParams() as $key => $value) {
            $checkItem = \App\Model\Config::where('name', $key)->first();
            if ($checkItem) {
    ...
    {
        // Check if Exists
        $group = \App\Model\ConfigGroups::find($value->id);
        ...
        if (!$group) {
            // Create Group
            $group = new \App\Model\ConfigGroups;
            $group->name = $value->name;
            $group->description = $value->description;
            $group->page_name = $value->page_name;
            $group->save();
    ...
    private function importConfig($group, $config, $overwrite = 0)
    {
        ...
        foreach ($config as $cfg) {
            ...
            $config = \App\Model\Config::where('name', $cfg->name)->where('group_id', $group->id)->first();
