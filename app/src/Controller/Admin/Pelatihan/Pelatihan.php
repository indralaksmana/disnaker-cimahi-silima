...
class Pelatihan extends Controller
{
    ...
    public function pelatihan(Request $request, Response $response)
    {
        ...
        $posts = PelatihanPosts::with('category')->where('status',1)->withCount('peserta');

        $laporan = PelatihanLaporanAkhirModel::with('pelatihan')->get();
        $postsclosed = PelatihanPosts::with('category')->where('status',0)->withCount('peserta');
        ...
        return $this->view->render(
            $response,
            'dashboard/pelatihan/pelatihan.twig',
            array(
                "categories" => PelatihanCategories::get(),
                "tags" => PelatihanTags::get(),
                "posts" => $posts->get(),
                "laporan" => $laporan,
                "postsclosed" => $postsclosed->get()
            )
        );
    }
    ...
    public function dataTables(Request $request, Response $response)
    {
        ...
        $posts = PelatihanPosts::select(
            ...
            'c_pp_c_pp_pelatihan_categories.name as category'
        )
            ->leftJoin('c_pp_pelatihan_categories', 'pelatihan_posts.category_id', '=', 'c_pp_pelatihan_categories.id')
            ->orderBy($order, $dir)
            ->skip($start)
            ->take($limit);
        ...
        if (!empty($request->getParam('search')['value'])) {
            ...
            $posts =  $posts->where('pelatihan_posts.title', 'LIKE', "%{$search}%")
                    ...
                    ->orWhere('c_pp_c_pp_pelatihan_categories.name', 'LIKE', "%{$search}%");
    ...
    {
        if ($check = $this->sentinel->hasPerm('pelatihan.create', 'dashboard', $this->config['pelatihan-enabled'])) {
            return $check;
        }

        if ($request->isPost()) {
            if ($this->pelatihanUtils->addPost()) {
                return $this->redirect($this->container->response, 'admin-pelatihan');
            }
        }

        return $this->view->render(
            $response,
            'dashboard/pelatihan/pelatihan-add.twig',
            array(
                "categories" => PelatihanCategories::get(),
                "tags" => PelatihanTags::get()
            )
    ...
    public function pelatihanEdit(Request $request, Response $response, $postId)
    {
        ...
        return $this->view->render(
            $response,
            'dashboard/pelatihan/pelatihan-edit.twig',
            array(
                "post" => $post->toArray(),
                "categories" => PelatihanCategories::get(),
                "tags" => PelatihanTags::get(),
