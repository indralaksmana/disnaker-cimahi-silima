 ...
 */
class UsersBKK extends Controller
{
    ...
    public function usersAdd(Request $request, Response $response)
    {
        ...
        return $this->view->render(
          $response,
         'dashboard/admin/usersmanagement/users/users-add-bkk.twig',
           ["bkk" => $bkk]
         );
    }
    ...
    private function addUser()
    {
        ...
        $user = $this->auth->registerAndActivate([
            ...
            'bkk_id' => $this->request->getParam('bkk_id'),
            'permissions' => [
                'user.delete' => 0
            ]
        ]);

        $role = $this->auth->findRoleBySlug('bkk');
    ...
    private function editUser($user = null)
    {
      ...
      $user->email = $this->request->getParam('email');
      $user->username = $this->request->getParam('username');
      $user->bkk_id = $this->request->getParam('bkk_id');
      $user->save();
      $role = $this->auth->findRoleBySlug('bkk');
    ...
    public function usersEdit(Request $request, Response $response, $userid)
    {
        ...
        return $this->view->render(
          $response,
          'dashboard/admin/usersmanagement/users/users-edit-bkk.twig',
          [
            'user' => $user,
            'bkk' => $bkk
          ]
