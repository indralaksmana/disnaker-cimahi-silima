...
class Lowongan extends Controller
{
    ...
    public function job(Request $request, Response $response)
    {
        ...
        return $this->view->render(
            $response,
            'bkol/dashboard/lowongan/list.twig',
            array(
                "posts" => $posts->get()
            )
        );
    ...
    public function jobAdd(Request $request, Response $response)
    {
        ...
        if ($request->isPost()) {
            if ($this->jobUtils->addPost()) {
                if ($this->auth->check()->inRole('pemerintah')) {
                    return $this->redirect($this->container->response, 'dashboardpemerintah-lowongan');
                }
        ...
        return $this->view->render(
            $response,
            'bkol/dashboard/lowongan/add-edit.twig',
            array(
    ...
    public function jobEdit(Request $request, Response $response, $postId)
    {
        ...
        if ($check = $this->sentinel->hasPerm('job.update')) {
            return $check;
        }
        ...
        if (!$post) {
            ...
            if ($this->auth->check()->inRole('pemerintah')) {
                return $this->redirect($response, 'dashboardpemerintah-lowongan');
            }
            if ($this->auth->check()->inRole('companies')) {
