...
class DashboardPerusahaan extends Controller
{
    ...
    public function SettingsAkun(Request $request, Response $response)
    {
        ...
        return $this->view->render(
            $response, 'bkol/dashboard-perusahaan/pengatuan-akun.twig'
        );
    }
        ...
        if ($request->isPost()) {
            ...
            //if ($this->validator->isValid()) {
                if ($this->updateAkun($request->getParams())) {
                    ...
                    return $this->redirect($response, 'dashboardperusahaan-settings-akun');
                }
    ...
    public function ProfilPerusahaan(Request $request, Response $response)
    {
        if ($check = $this->sentinel->hasPerm('companies.account')) {
            return $check;
        }
        ...
        if ($request->isPost()) {
            ...
            $this->validator->validate($request, $validateData);
            if ($this->validator->isValid()) {
                if ($this->updatePerusahaan($request->getParams())) {
                    $this->flash('success', 'Account anda telah diperbarui dengan sukses.');
    ...
    private function updatePerusahaan($requestParams)
    {
        ...
        if ($updateCompanies) {
            ...
            if (isset($files['logo_upload']->file) && $files['logo_upload']->file != '') {
              $updateCompanies->logo = $this->upload_image($files['logo_upload']->file, '/files/perusahaan/logo');
            } else {
              $updateCompanies->logo = isset($requestParams['logo']) ? $requestParams['logo'] : NULL;
            }
        ...
        if ($addCompanies || $updateCompanies) {
            return true;
        }
        return false;
    }
