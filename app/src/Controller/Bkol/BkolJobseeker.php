...
class BkolJobseeker extends Controller
{
    /**
    ...
    public function main(Request $request, Response $response)
    {
        ...
        $pencarikerja = BkolPencariKerja::has('user')->with('userpencarikerja')->get();
        return $this->view->render(
            $response,
            'bkol/pencarikerja/pencarikerja.twig',
            array(
    ...
    public function Detail(Request $request, Response $response, $args)
    {
        ...
        $post = BkolPencariKerja::with(
            'userpencarikerja',
            'userpencarikerja.riwayatpekerjaan',
            'userpencarikerja.riwayatpendidikan',
            'userpencarikerja.riwayatnonformalpendidikan')
            ->where('ak1_id', $args['id'])->first();
        if (!$post) {
        ...
        if ($request->isPost()) {
            if (!$this->auth->check()) {
                ...
                return $this->view->render($response, 'bkol/pencarikerja/pencarikerja-detail.twig', array("post" => $post, "showSidebar" => 1));
            }
        ...
        return $this->view->render(
            $response,
            'bkol/pencarikerja/pencarikerja-detail.twig',
            array(
              "r" => $post,
              "s" => $shortlist,
