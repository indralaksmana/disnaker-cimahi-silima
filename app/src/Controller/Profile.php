...
class Profile extends Controller
{
   /**
        ...
        if ($request->isPost()) {
            if ($request->getParam('save_profile') !== null) {
                ...
                if ($this->validator->isValid()) {
                    ...
                    $updateProfile = Companies::where('user_id', $user->id)->first();

                    if (!$updateProfile) {
                        $updateProfile = new Companies;
                        $updateProfile->user_id = $user->id;
                        $updateProfile->save();
                    }
    ...
    public function profileIncomplete(Request $request, Response $response)
    {
        ...
        return $this->view->render($response, 'disnaker/profile-incomplete.twig', array("user" => $user));
    }
